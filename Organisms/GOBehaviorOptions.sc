GOBehaviorOptions{
	...
	classvar <>impulseRateHiMin = 12.501, <>impulseRateHiMax = 250.0;
	...
	classvar <>timescaleMin = 0.25, <timescaleMax = 4.0;
	...
	*new{
	...
	averageOptions{|targetOptions|
		...
		return.rateLo = (rateLo + targetOptions.rateLo) / 2;
		return.rateHi = (rateHi + targetOptions.rateHi) / 2;
		return.posLo = (posLo + targetOptions.posLo) / 2;
		return.posHi = (posHi + targetOptions.posHi) / 2;
		return.ampLo = (ampLo + targetOptions.ampLo) / 2;
		return.ampHi = (ampHi + targetOptions.ampHi) / 2;
		return.ffreqLo = (ffreqLo + targetOptions.ffreqLo) / 2;
		return.ffreqHi = (ffreqHi + targetOptions.ffreqHi) / 2;
		return.impulseRateLo = (impulseRateLo + targetOptions.impulseRateLo) / 2;
		return.impulseRateHi = (impulseRateHi + targetOptions.impulseRateHi) / 2;
		return.grainDurLo = (grainDurLo + targetOptions.grainDurLo) / 2;
		return.grainDurHi = (grainDurHi + targetOptions.grainDurHi) / 2;
		return.rqLo = (rqLo + targetOptions.rqLo) / 2;
		return.rqHi = (rqHi + targetOptions.rqHi) / 2;
		return.timescale = (timescale + targetOptions.timescale) / 2;
